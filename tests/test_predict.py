...
@pytest.mark.asyncio
async def test_predict_violation_true(auth_client: AsyncClient):
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_predict_violation_false(auth_client: AsyncClient):
    response = await auth_client.post(
        "/predict",
        json={
            "seller_id": 2,
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_validation_errors(auth_client: AsyncClient, invalid_data, description):
    response = await auth_client.post("/predict", json=invalid_data)
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_simple_predict_positive(auth_client: AsyncClient, db_connection):
    user_id = await UserRepository.create_user(is_verified=False)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Suspicious",
    ...
    response = await auth_client.post(f"/predict/simple_predict?item_id={item_id}")
    assert response.status_code == 200
    data = response.json()
    assert "is_violation" in data
    assert "probability" in data
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_simple_predict_negative(auth_client: AsyncClient, db_connection):
    user_id = await UserRepository.create_user(is_verified=True)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Good Item",
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_simple_predict_not_found(auth_client: AsyncClient, db_connection):
    response = await auth_client.post("/predict/simple_predict?item_id=999999")
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_async_predict_creates_task_and_sends_message(auth_client: AsyncClient, fake_kafka_producer, db_connection):
    user_id = await UserRepository.create_user(is_verified=False)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Async Item",
    ...
    resp = await auth_client.post("/async_predict", json={"item_id": item_id})
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_moderation_result_by_task_id(auth_client: AsyncClient, db_connection):
    user_id = await UserRepository.create_user(is_verified=False)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Async Item 2",
    ...
    create_resp = await auth_client.post("/async_predict", json={"item_id": item_id})
    task_id = create_resp.json()["task_id"]
    status_resp = await auth_client.get(f"/moderation_result/{task_id}")
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_worker_process_one_completes_task(monkeypatch, auth_client: AsyncClient, db_connection):
    user_id = await UserRepository.create_user(is_verified=True)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Good item",
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_login_success(client: AsyncClient, db_connection):
    reg_resp = await client.post("/auth/register", json={"login": "user", "password": "pass"})
    assert reg_resp.status_code == 200

    resp = await client.post("/auth/login", json={"login": "user", "password": "pass"})
    assert resp.status_code == 200
    data = resp.json()
    ...
    assert cookies["access_token"] == data["access_token"]


@pytest.mark.integration
@pytest.mark.asyncio
async def test_login_wrong_password(client: AsyncClient, db_connection):
    await client.post("/auth/register", json={"login": "user", "password": "pass"})
    resp = await client.post("/auth/login", json={"login": "user", "password": "wrong"})
...
@pytest.mark.integration
@pytest.mark.asyncio
async def test_login_nonexistent_user(client: AsyncClient, db_connection):
    resp = await client.post("/auth/login", json={"login": "no", "password": "pass"})
    assert resp.status_code == 401


@pytest.mark.integration
@pytest.mark.asyncio
async def test_predict_with_valid_token(auth_client: AsyncClient, db_connection):
    user_id = await UserRepository.create_user(is_verified=False)
    item_id = await ItemRepository.create_item(
        user_id=user_id,
        name="Test",
    ...
    resp = await auth_client.post("/predict", json=payload)
    assert resp.status_code == 200